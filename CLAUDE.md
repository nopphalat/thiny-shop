...
- **Delete-customer button actually works** — `admin-screens.jsx` EditCustomerModal. Previously called `DELETE /api/customers/:id` only, so "chat-only" customers (synthetic `GUEST-xxxx` ids built from chat_orders rows) reappeared on reload. Now: (1) finds all chat_orders linked to the customer by normalized phone or name, (2) DELETEs each via `/api/chat-orders/:id`, (3) DELETEs the customers row only when not chat-only, (4) shows alert + error handling, (5) `try/catch` around fetches with HTTP-status check. Bumped admin-screens.jsx v32→33.
